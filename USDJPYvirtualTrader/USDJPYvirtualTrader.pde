/*be careful of limits of calling rest api. 
 if over 500 per 5 minutes from same ip address,
...
void getCandles() {
  int i, j;
  String urls = "curl -H \"Authorization: Bearer 48db3546e09e037c829ce0507b4da944-0e75d408b9e2bb35a1e98a3409506871\" \"https://api-fxpractice.oanda.com/v3/accounts/101-001-12970645-001/instruments/GBP_JPY/candles?granularity=M1&count=90\"";
  String outFile = " > " + cd + "\\ohlc.json";
  String ohlc_data = cd + "\\ohlc.json";
  ...
  for (i=0; i<=89; i++) {
    println("open  : " + opens.get(i));
    println("high  : " + highs.get(i));
    println("low   : " + lows.get(i));
    println("close : " + closes.get(i));
    println("");
  }
}
...
void getBalance() {
  int[] tmp;
  File balance_file = new File(cd +  "\\balance.txt");
  println(cd + "\\balance.txt");
...
void getMarketData() {
  ...
  String urls = "curl -H \"Authorization: Bearer 48db3546e09e037c829ce0507b4da944-0e75d408b9e2bb35a1e98a3409506871\" \"https://api-fxpractice.oanda.com/v3/accounts/101-001-12970645-001/pricing?instruments=GBP_JPY\"";
  String outFile = " > " + cd + "\\ticker.json" ;
  String getPrice = urls + outFile;
  String ticker = cd + "\\ticker.json";  // file name of ticker data
...
void drawStats() {
  ...
  for (int i = 0; i < num_order; i++) {
    ...
    textAlign(LEFT, TOP);
    text("USDJPY" + space + BS + space + "at" + space + in_price[i] + space + "x" + space + lots[i] + space + "profit: " + float_profits[i], 10, y);
    textAlign(CENTER, TOP);    
    text("CLOSE", 1800, y);
    y += 30;
  }
...
void drawChart() {
  ...
  for (int i = 0; i < 90; i++) {
    //get distance(px) from height of window
...
void getPriceHeightPerPx() {  
  ...
  println("price per px : " + price_per_px);
  println("price range :" + price_per_px * 640);
}
  ...
  if (lot > 0 && num_order < 5) {  // when lot > 0 number of orders under 5
    ...
    if (mouseX >= 0 && mouseX <= 249 && mouseY >= 40 && mouseY <= 160 || mouseX >= 0 && mouseX <= 150 && mouseY >= 0 && mouseY <= 39) {    // when bid is pressed = sell
      ...
      if (equity - (margin_sum + (bid * lot *lot_size / leverage)) < 0) {
        ...
        text("WARING: " + "margin isn't enough ", 1590, 160);
        return;
      }
